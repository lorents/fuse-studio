...
namespace Outracks.Fuse.Toolbox
{
	...
	public static class Toolbox
	{
		...
		public static IControl Create(ProjectModel project, ElementContext elementContext)
		{
			var m = Spacer.Medium.DesiredSize.Width;
			var s = Spacer.Small.DesiredSize.Width;

			var hasClasses = project.Classes.Any();
			...
			return Layout.Dock()
				...
				.Fill(
					//project.Classes
					//	.Select(e => CreateClassItem(e, project.Context, elementContext, searchString))
					//	.StackFromTop()
		...
		{
			var className = classElement["ux:Class"];
			return Layout
				.StackFromLeft(
						classElement.MediumIcon(Theme.Purple, Theme.IconSecondary).CenterVertically(),
						Spacer.Small,
						Label.Create(className.StringValue.AsText(), font:Theme.DefaultFont, color: Theme.DefaultText).CenterVertically())
				...
				.WithBackground(Shapes
					...
					//.ShowWhen(context.IsSelected(classElement.IsSelected))
					)
				.DockLeft()
				.OnMouse(
					...
					dragged: className.StringValue.Select(er => SourceFragment.FromString("<" + er + "/>"))//,
					//doubleClicked: Command.Enabled(() =>
					//	context.SetScope(root: classElement, selection: classElement))
					)
				//TODO.SetContextMenu(elementContext.CreateMenu(classElement))
				.WithHeight(26)
				.ShowWhen(
					Observable.CombineLatest(
						searchString,
						className.StringValue.AsText(),
						(search, name) =>
						{
