...
namespace Outracks.Fuse.Tests.Hierarchy
{
	...
	//		_element = new RootElement(null);//LiveElementFactory.CreateLiveElement("<App><StackPanel/></App>", _path, invalidated);
	//		//UpdateElementIds();
	...
	//		//	{
	//		//		UpdateElementIds();
	//		//		return Element.Empty;  //TODO: _element.DescendantsAndSelf().Where(x => x.SimulatorId.Value == id).FirstOr(Element.Empty);
	//		//	});
	...
	//	WeakReference<ElementModel> GetElementWeakReferenceByName(string name)
	//	{
	//		return new WeakReference<ElementModel>(GetElementByName(name));
	//	}

	//	ElementModel GetElementByName(string name)
	//	{
	//		return _elementDescendantsAndSelf().First(x => x.Name.First() == name);
	//	}
