	...
	public class ProxyServer : IDisposable
	{
		...
		public void AddProject(int port, string code, string projectPath, string[] defines)
		{
			lock (_projects)
			{
				_projects[port] = new ProxyProjectEntry
				{
					Port = port,
					Code = code,
					Project = Uno.ProjectFormat.Project.Load(projectPath),
					Defines = defines,
				};
			}
		}
		...
		Optional<ProxyProjectEntry> GetMatchingProject(string path, string code, string[] defines)
		{
			...
			foreach (var entry in entries)
			{
				...
				{
					_logWarning("Warning: The preview running on the device has a different set of defines than the one running on your computer. This is normally ok, and expected for cases like `-DGRADLE`.");
